...
def start_good(text):
    SvgDrawer.draw(get_letter_path(text))
    if SvgDrawer.communicate_with_arduino:
        config.rope_left = SvgDrawer.ArduinoController.l_1_i
        config.rope_right = SvgDrawer.ArduinoController.l_2_i
...
def launch():
    ...
    if str(art) != "":
        ...
        print("error")
    SvgDrawer.ArduinoController.up()
    SvgDrawer.ArduinoController.send_coordinates((0, 0))
    t.mainloop()
